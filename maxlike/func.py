...
class Comp(Func):
    def __init__(self, f, g):
        """
        Generates a new function:   f o g
        """
        self.f = f
        self.g = g
    ...
    @call_func
    def __call__(self, params, **kwargs):
        return self.f(self.g(params, **kwargs))

    @vector_func
    def grad(self, params, i, **kwargs):
        g0 = self.g(params)
        n_i = params[i].ndim
        idx = [None] * n_i + [Ellipsis]
        return l_mul(self.f.grad(g0, 0)[idx], self.g.grad(params, i)).\
            sum(tuple(range(n_i, n_i + g0.ndim)))

    @matrix_func
    def hess(self, params, i, j, **kwargs):
        g0 = self.g(params, **kwargs)
        n_g = g0.ndim
        n_i = params[i].ndim
        n_j = params[j].ndim
        idx = [None] * (n_i + n_j) + [Ellipsis]
        M1 = self.f.hess(g0, 0, 0)[idx]
        M2 = l_mul(self.f.grad(g0, 0)[idx], self.g.hess(params, i, j))
        idx = [slice(None)] * n_i + [None] * n_j + [Ellipsis]
        M1 = l_mul(M1, self.g.grad(params, i)[idx])
        M1 = l_mul(M1, self.g.grad(params, j)[[None] * n_i + [Ellipsis]])
        M1 = M1.sum(tuple(range(n_i + n_j, n_i + n_j + 2 * n_g)))
        M2 = M2.sum(tuple(range(n_i + n_j, n_i + n_j + n_g)))
        return M1 + M2
...
if __name__ == "__main__":
    e = Exp()
    poi = PoissonVector(10)
    #F = Comp(poi, e)
    x = np.array([-.5, 1])
    print((e.hess(x)))
    #print(s.hess(p, 0, 0).shape)
    #s = Sum(3)
